...
func init() {
	...
	sessionStartCmd.Flags().StringVar(&sessionAgent, "agent", "", "Agent override for this session (e.g., codex, gemini)")
	...
	sessionRestartCmd.Flags().StringVar(&sessionAgent, "agent", "", "Agent override for this session (e.g., codex, gemini)")
...
func runSessionRestart(cmd *cobra.Command, args []string) error {
	...
	// Start fresh session. Model resolution is centralized in
	// SessionManager.Start: explicit --agent > persisted assigned_agent >
	// rig/town default. Warn if the restart falls through to the default so
	// operators can detect potential model drift from a missing persistence
	// record (gastown-hkd).
	fmt.Printf("Starting session for %s/%s...\n", rigName, polecatName)
	opts := polecat.SessionStartOptions{
		Agent:                 sessionAgent,
		WarnIfDefaultFallback: sessionAgent == "",
	}
	if err := polecatMgr.Start(polecatName, opts); err != nil {
		return fmt.Errorf("starting session: %w", err)
	}
