...
// stamped (5/5 hardening-batch polecats hit this before the fix).
//
	...
	if hookedBeadID != "" && (exitType != ExitDeferred || isWorkflowStep) {
		...
		if hookedBead, err := bd.Show(hookedBeadID); err == nil && !beads.IssueStatus(hookedBead.Status).IsTerminal() {
			...
			if unchecked := beads.HasUncheckedCriteria(hookedBead); unchecked > 0 {
				...
				// gastown-5oy: When an MR was successfully submitted (MRID set in
				// the agent bead's CompletionMetadata, no failures), defer the
				// source-bead close to the refinery. The refinery stamps
				// "Merged in <MR-id>" provenance when it merges the MR. Closing
				// here with the default "Closed" reason would beat the refinery
...
func mrContextForDeferral(agentBd *beads.Beads, agentBeadID, exitType string) (string, bool) {
	...
	// MRID is the primary signal; active_mr is the same value (set earlier in
	// runDone) and serves as a cross-check. If either disagrees with reality
	// we don't defer — fail-safe to immediate close.
	if fields.MRID == "" || fields.ActiveMR == "" || fields.MRID != fields.ActiveMR {
		return "", false
	}
