...
// TestReadConvoyIssues_EmptyOutputTreatedAsNoIssues verifies that bd list
// returning absolutely nothing (empty stdout) is treated as "no issues"
// instead of aborting the staging pipeline with an "unexpected end of JSON
// input" error. This regressed during retro-audit gastown-cet.12.2 and
// caused convoy-stage --json tests to emit empty stdout (gastown-l76).
func TestReadConvoyIssues_EmptyOutputTreatedAsNoIssues(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping bd stub test on Windows")
	}
	...
	// Mock bd list returns empty output for the convoy query.
	script := `#!/bin/sh
case "$*" in
  *list*--label=gt:convoy*)
    # Emit nothing, as real bd can do when no convoys exist.
    exit 0
    ;;
  *)
    echo '[]'
...
`
	...
	issues, err := readConvoyIssues(townRoot, "list", "--label=gt:convoy", "--json", "--limit=0", "--all")
	if err != nil {
		t.Fatalf("readConvoyIssues() error = %v, want nil for empty output", err)
	}
	if len(issues) != 0 {
		t.Errorf("readConvoyIssues() returned %d issues, want 0 for empty output", len(issues))
	}
}

// TestReadConvoyIssues_NonJSONTextTreatedAsNoIssues verifies that plain-text
// responses like "No issues found." from bd list are treated as "no issues"
// rather than fatal JSON parse errors (gastown-l76).
func TestReadConvoyIssues_NonJSONTextTreatedAsNoIssues(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("skipping bd stub test on Windows")
	}
	...
	// Mock bd list returns human-friendly text instead of JSON.
	script := `#!/bin/sh
case "$*" in
  *list*--label=gt:convoy*)
...
`
	...
	issues, err := readConvoyIssues(townRoot, "list", "--label=gt:convoy", "--json", "--limit=0", "--all")
	if err != nil {
		t.Fatalf("readConvoyIssues() error = %v, want nil for non-JSON text", err)
	}
	if len(issues) != 0 {
		t.Errorf("readConvoyIssues() returned %d issues, want 0 for non-JSON text", len(issues))
	}
}
