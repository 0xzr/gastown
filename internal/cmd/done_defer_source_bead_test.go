...
func TestMrContextForDeferralLogic(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "active_mr and mr_id disagree → skip",
			desc:      "agent_state: working\nactive_mr: gastown-wisp-a\nexit_type: COMPLETED\nmr_id: gastown-wisp-b\nmr_failed: false\npush_failed: false",
			exitType:  ExitCompleted,
			wantDefer: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotDefer := shouldDefer(af, tt.exitType)
			if gotDefer != tt.wantDefer {
				t.Errorf("shouldDefer = %v, want %v", gotDefer, tt.wantDefer)
			}
		})
	}
}

// shouldDefer mirrors mrContextForDeferral's pure decision so it can be
// unit-tested without a bd stub. Extracted here to keep the helper focused
// on I/O and the policy here for direct table coverage.
func shouldDefer(af *beads.AgentFields, exitType string) bool {
	if af == nil || exitType != ExitCompleted {
		return false
	}
