...
func TestDoltFlatten_RejectsMaliciousDBName(t *testing.T) {
	...
	for _, name := range cases {
		t.Run(name, func(t *testing.T) {
			out, err := executeCmdTree(t, "dolt", "flatten", "--yes-i-am-sure", name)
			if err == nil {
...
func TestDoltRebase_RejectsMaliciousDBName(t *testing.T) {
	...
	for _, name := range cases {
		t.Run(name, func(t *testing.T) {
			out, err := executeCmdTree(t, "dolt", "rebase", "--yes-i-am-sure", name)
			if err == nil {
...
// executeCmdTree runs `gt <args...>` through the cobra tree and returns both
...
func executeCmdTree(t *testing.T, args ...string) (string, error) {
	t.Helper()
	buf := &bytes.Buffer{}
	rootCmd.SetOut(buf)
	rootCmd.SetErr(buf)
