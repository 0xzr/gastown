...
// keep individual delete statements bounded. After the loop completes the
// batched deletes are committed via DOLT_COMMIT.
//
...
func CleanDanglingParentRefs(db *sql.DB, dbName string, dryRun bool) (*DanglingCleanResult, error) {
	...
	// the reported count equals the count that gets deleted.
