...
// surface leaf strings (callers should use carrierText / nestedString for
// that, which also handle string-vs-object carriers).
...
// extractFromPartText (string OR object with nested .text).
...
// structured JSON object. If .result is an object, the carrier walks
// .verdict → .text → .raw_output → JSON-marshal-the-whole-object in priority
// order, mirroring the live gate's loose `.verdict // .review.verdict //
// .raw_output // .text` fallback chain applied to top-level adapter output.
...
//   - a JSON object, in which case the carrier recursively tries `.text`,
//     `.verdict`, `.raw_output` in that order (mirroring the live gate's
//     object-shaped support for part.text / item.text / .result).
//
// Returns ("", false) on any miss — missing key, wrong type, empty string,
// no nested-text path inside an object carrier. Callers pipe the result
...
func carrierTextOrNested(entry map[string]json.RawMessage, path ...string) (string, bool) {
	...
	// Object leaf — walk nested .text / .verdict / .raw_output.
