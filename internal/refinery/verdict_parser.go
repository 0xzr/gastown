...
func ExtractVerdictFromText(text string) VerdictExtraction {
	...
	// Scan ALL matches and return the LAST parseable verdict (last-wins).
	var last string
	for _, m := range verdictRe.FindAllStringSubmatch(text, -1) {
		if len(m) >= 2 {
			last = strings.ToUpper(m[1])
		}
	}
	if last != "" {
		return VerdictExtraction{Verdict: ReviewerVerdict(last), Source: VerdictSourceVerdictRegex}
	}
