...
func (e *Engineer) doMerge(ctx context.Context, branch, target, sourceIssue string, mr *MRInfo, skipGates ...bool) ProcessResult {
	...
	// Revalidate the pre-verified fast-path against the *actual* refreshed
	// target commit (gastown-6n7, codex blocker #3). The skipGates decision
	// handed in from ProcessMRInfo is only a probe: it was computed against the
	// local origin/<target> ref BEFORE this checkout+pull refreshed the target.
	// If the remote-tracking ref was stale, or if target advanced between that
	// check and the pull above, the incoming skipGates may be true even though
	// the merge is now landing on a different base commit. Skipping gates in
	// that case would skip deterministic pre-merge/post-squash gates — and key
	// the durable gate off a stale skipGates=true — for a candidate that was
	// NOT actually pre-verified against its refreshed base.
	//
	// The authoritative decision belongs here, where the refreshed reviewBase
	// is known. shouldSkipGates is true only if the MR is pre-verified, carries
	// a PreVerifiedBase, AND that base is exactly the refreshed target commit.
	// A stale pre-verified MR (target moved) becomes shouldSkipGates=false, so
	// deterministic gates run and the durable gate treats it as NOT on the
	// fast path — consistent with the durableReviewRequiredForMR contract.
	shouldSkipGates := false
	if len(skipGates) > 0 && skipGates[0] && mr != nil && mr.PreVerified && mr.PreVerifiedBase != "" {
		if reviewBase == mr.PreVerifiedBase {
			shouldSkipGates = true
		} else {
			_, _ = fmt.Fprintf(e.output, "[Engineer] Pre-verification stale after target refresh — reviewBase %s != PreVerifiedBase %s; running gates normally\n",
				reviewBase[:min(8, len(reviewBase))], mr.PreVerifiedBase[:min(8, len(mr.PreVerifiedBase))])
		}
	}
	...
	// Phase 3 fast-path: shouldSkipGates was revalidated above against the
	// refreshed reviewBase (gastown-6n7). If true, skip all gate execution —
	// the polecat already ran gates after rebasing onto this exact base.
	if shouldSkipGates {
