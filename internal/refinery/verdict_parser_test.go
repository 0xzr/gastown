...
// gastown-wisp-2ds regression: ExtractVerdictFromText must not stop at the
// first fenced verdict. A fenced PASS followed by a later unfenced FAIL must
// return FAIL per the parser's last-wins recovery contract.
func TestExtractVerdictFromText_FencedPASS_LaterUnfencedFAIL_LastWins(t *testing.T) {
	in := fencedJSONPassPayload + `

Later, an unfenced JSON object overrides the earlier fenced verdict: {"verdict":"FAIL"}`
	got := ExtractVerdictFromText(in)
	if got.Verdict != ReviewerVerdictFail {
		t.Errorf("got %v, want FAIL — fenced PASS followed by later unfenced FAIL must be last-wins (source=%q)", got.Verdict, got.Source)
	}
}
...
func TestExtractVerdictFromText_RegexFallback_LastWins_PASS_then_FAIL(t *testing.T) {
	in := `"verdict":"PASS" ... "verdict":"FAIL"`
	got := ExtractVerdictFromText(in)
	if got.Verdict != ReviewerVerdictFail {
		t.Errorf("got %v, want FAIL — regex fallback must be last-wins", got.Verdict)
	}
}

func TestExtractVerdictFromText_RegexFallback_LastWins_FAIL_then_PASS(t *testing.T) {
	in := `"verdict":"FAIL" ... "verdict":"PASS"`
	got := ExtractVerdictFromText(in)
	if got.Verdict != ReviewerVerdictPass {
		t.Errorf("got %v, want PASS — regex fallback must be last-wins", got.Verdict)
	}
}
...
func TestExtractVerdictFromJSONL_TextEventPriorityOverWrapperDisagreement(t *testing.T) {
	// Higher carrier priority wins when both carriers agree on PASS and no
	// real FAIL is present. part.text PASS > findings[].raw_output PASS.
	log := strings.Join([]string{
		jsonlTextEvent("```json\n{\"verdict\":\"PASS\"}\n```"),
		jsonlSyntheticReviewResult(fencedJSONPassPayload),
	}, "\n")
	...
	if got.Verdict != ReviewerVerdictPass {
		t.Errorf("got %v, want PASS — higher priority (part.text) PASS should win when no real FAIL exists", got.Verdict)
	}
...
func TestExtractVerdictFromJSONL_TextEventPASS_DoesNotMask_NonSyntheticReviewFAIL(t *testing.T) {
	// Core gastown-wisp-wdm regression: a higher-priority real PASS carrier
	// (part.text) must NOT mask a lower-priority real FAIL carrier
	// (non-synthetic part.review.verdict). Fail-closed → FAIL.
	log := strings.Join([]string{
		jsonlTextEvent("```json\n{\"verdict\":\"PASS\"}\n```"),
	...
	if got.Verdict != ReviewerVerdictFail {
		t.Errorf("got %v, want FAIL — real FAIL at lower priority must not be masked by higher-priority PASS (gastown-wisp-wdm)", got.Verdict)
	}
...
func TestExtractVerdictFromJSONL_FindingsPASS_DropsSyntheticWrapperVerdictFAIL(t *testing.T) {
	// Synthetic wrapper's own verdict FAIL is noise; the real model text in
	// findings[].raw_output carries PASS. Result must be PASS.
	log := strings.Join([]string{
		jsonlSyntheticReviewResult(fencedJSONPassPayload),
	}, "\n")
	got, err := ExtractVerdictFromJSONL(strings.NewReader(log))
	...
	if got.Verdict != ReviewerVerdictPass {
		t.Errorf("got %v, want PASS — synthetic wrapper FAIL must be dropped when findings[].raw_output is PASS", got.Verdict)
	}
}
...
func TestExtractVerdictFromJSONL_LastRecordWinsWithinPriority(t *testing.T) {
	...
	// wins when both agree; if a real FAIL appears anywhere it wins first.
	fencedA := "```json\n{\"verdict\":\"PASS\"}\n```"
	fencedB := "```json\n{\"verdict\":\"FAIL\"}\n```"
	log := strings.Join([]string{
...
func TestExtractVerdictFromJSONL_HigherPriorityBeatsLaterRecord_NoFAIL(t *testing.T) {
	// part.text PASS (record 0, prio=partText) beats part.review.verdict
	// PASS (record 1, prio=partReviewVerdict) when no FAIL exists. Higher
	// priority PASS wins regardless of record order.
	log := strings.Join([]string{
		jsonlTextEvent("PASS"),
	...
	if got.Verdict != ReviewerVerdictPass {
		t.Errorf("got %v, want PASS — higher priority (part.text) should beat later lower-priority PASS", got.Verdict)
	}
}
...
func TestExtractVerdictFromJSONL_StructuredResult_FallsThroughToRawOutput(t *testing.T) {
	...
	if got.Verdict != ReviewerVerdictPass {
		t.Errorf("got %v, want PASS — when .verdict/.text missing, parser must fall through to .raw_output", got.Verdict)
	}
}
...
func TestExtractVerdictFromJSONL_SyntheticSkipsOwnPartReviewVerdict(t *testing.T) {
	// A synthetic wrapper's OWN part.review.verdict is a fixed FAIL wrapper
	// signal and must be dropped. The real model text preserved in
	// findings[].raw_output is PASS, so the overall verdict is PASS.
	log := strings.Join([]string{
		jsonlSyntheticReviewResult(fencedJSONPassPayload),
	}, "\n")
	got, err := ExtractVerdictFromJSONL(strings.NewReader(log))
	...
	if got.Verdict != ReviewerVerdictPass {
		t.Errorf("got %v, want PASS — synthetic wrapper's own verdict must be dropped", got.Verdict)
	}
}

func TestExtractVerdictFromJSONL_Synthetic_DoesNotMask_RealNonSyntheticFAIL(t *testing.T) {
	// Comprehensive-rewrite invariant: the synthetic exception is NARROW.
	// A non-synthetic review_result with a real FAIL is real evidence and
	// wins over a synthetic wrapper that reports PASS via findings[].raw_output.
	log := strings.Join([]string{
		jsonlSyntheticReviewResult(fencedJSONPassPayload),
		jsonlNonSyntheticReviewVerdict("FAIL"),
	}, "\n")
	...
	if got.Verdict != ReviewerVerdictFail {
		t.Errorf("got %v, want FAIL — non-synthetic FAIL must not be masked by synthetic PASS (gastown-wisp-wdm)", got.Verdict)
	}
}
...
func TestExtractVerdictFromJSONL_NonSyntheticNoSkipsPartReviewVerdict(t *testing.T) {
	...
	// Real FAIL at part.review.verdict beats real PASS at higher-priority
	// part.text because fail-closed conflict resolution prioritizes FAIL.
	if got.Verdict != ReviewerVerdictFail {
		t.Errorf("got %v, want FAIL", got.Verdict)
	}
}
...
func TestExtractVerdictFromJSONL_LooseFallbackSyntheticReviewVerdict_Dropped(t *testing.T) {
	// A synthetic wrapper's .review.verdict (loose-fallback shape) is the
	// same wrapper signal as .part.review.verdict and must be dropped.
	// We also put a top-level .verdict=PASS to prove the synthetic exception
	// is narrow: top-level .verdict is kept real and contributes PASS, while
	// .review.verdict (the wrapper path) is dropped.
	line := map[string]any{
		"type":    "review_result",
		"verdict": "PASS",
		"review": map[string]any{
	...
	if got.Verdict != ReviewerVerdictPass {
		t.Errorf("got %v, want PASS — synthetic wrapper .review.verdict must be dropped; top-level .verdict PASS kept real", got.Verdict)
	}
}
