...
// TestExtractVerdictFromJSONL_MultiFinding_LastVerdictWins covers a synthetic
// wrapper that emits multiple findings, only one of which carries a parseable
// verdict. The findings[] priority is consulted ONLY when at least one
// record is a synthetic OpenCode wrapper; here the wrapper carries
// adapter_error="no_parseable_review_json" so findings[].raw_output applies,
// and the LAST finding that yields a verdict wins (mirrors the live gate's
// extract_review_verdict_text() reading the concatenated jq output
// end-to-end and returning the last parsed verdict).
func TestExtractVerdictFromJSONL_MultiFinding_LastVerdictWins(t *testing.T) {
	// Build a synthetic wrapper with two findings — the second carries the
	// fenced PASS; the first carries prose that yields no verdict. With only
	// one parseable finding, FIRST and LAST wins coincide; the test name
	// documents the LAST-wins contract enforced by
	// extractFromPartReviewFindingsRawOutput.
	review := map[string]any{
		"verdict": "FAIL",
...
// TestExtractVerdictFromJSONL_LastRecordWinsWithinPriority — Finding 1 of the
// gastown-wisp-53s rejection. Two text events at the same carrier priority
// (part.text); the first carries an early PASS, the second a corrected FAIL.
// The previous FIRST-wins implementation wrongly accepted the early PASS;
// the corrected LAST-wins implementation honors the latest reviewer output,
// preserving fail-closed behavior when a model re-evaluates.
func TestExtractVerdictFromJSONL_LastRecordWinsWithinPriority(t *testing.T) {
	passLine := jsonlTextEvent("```json\n{\n  \"verdict\": \"PASS\"\n}\n```")
	failLine := jsonlTextEvent("```json\n{\n  \"verdict\": \"FAIL\",\n  \"blockers\": [\"race\"]\n}\n```")
	got, err := ExtractVerdictFromJSONL(strings.NewReader(passLine + "\n" + failLine + "\n"))
	if err != nil {
		t.Fatalf("ExtractVerdictFromJSONL: %v", err)
	}
	if got.Verdict != ReviewerVerdictFail {
		t.Fatalf("verdict = %q, want FAIL (last record at priority 1 wins)", got.Verdict)
	}
	if got.Source != VerdictSourceLogPartText {
		t.Errorf("source = %q, want %q", got.Source, VerdictSourceLogPartText)
	}

	// And the symmetric case — the LAST record PASSes after an early FAIL.
	got, err = ExtractVerdictFromJSONL(strings.NewReader(failLine + "\n" + passLine + "\n"))
	if err != nil {
		t.Fatalf("ExtractVerdictFromJSONL: %v", err)
	}
	if got.Verdict != ReviewerVerdictPass {
		t.Errorf("verdict = %q, want PASS (last record at priority 1 wins)", got.Verdict)
	}
}

// TestExtractVerdictFromJSONL_HigherPriorityBeatsLaterRecord — when records
// have DIFFERENT carrier priorities, the higher priority wins regardless of
// position. A later record's lower-priority verdict must NOT override an
// earlier record's higher-priority verdict. This is the cross-priority
// invariant the live gate enforces via its if-empty chain.
func TestExtractVerdictFromJSONL_HigherPriorityBeatsLaterRecord(t *testing.T) {
	passLine := jsonlTextEvent("```json\n{\n  \"verdict\": \"PASS\"\n}\n```")
	failReview := map[string]any{
		"verdict": "FAIL",
		"findings": []map[string]any{
			{"classification": "OK", "raw_output": "noise"},
		},
		"summary": "wrapper failed",
	}
	failLine, _ := json.Marshal(map[string]any{
		"type": "review_result",
		"part": map[string]any{
			"type":   "review-result",
			"review": failReview,
	...
	// Earlier PASS at priority 1 (text event); later FAIL at priority 5/6
	// (review_result). The priority-1 PASS must win — if a later lower-priority
	// verdict could mask an earlier higher-priority one, a malicious or buggy
	// adapter could downgrade a passing review by emitting an empty FAIL
	// envelope after.
	got, err := ExtractVerdictFromJSONL(strings.NewReader(passLine + "\n" + string(failLine) + "\n"))
	if err != nil {
		t.Fatalf("ExtractVerdictFromJSONL: %v", err)
	...
	if got.Verdict != ReviewerVerdictPass {
		t.Errorf("verdict = %q, want PASS (priority 1 beats later priority 6)", got.Verdict)
	}
	if got.Source != VerdictSourceLogPartText {
		t.Errorf("source = %q, want %q", got.Source, VerdictSourceLogPartText)
	}
...
// TestExtractVerdictFromJSONL_NestedObjectText — Finding 2 of the gastown-wisp-53s
// rejection. carrierText previously only accepted a flat string carrier; the
// live gate accepts part.text / item.text as EITHER a string OR an object
// whose .text field carries the text. This test asserts the object shape
// recovers the verdict (no longer drops it as no-verdict).
func TestExtractVerdictFromJSONL_NestedObjectText(t *testing.T) {
	// part.text as an object with nested .text
	line, _ := json.Marshal(map[string]any{
		"type": "text",
		"part": map[string]any{
			"type": "text",
	...
	if got.Verdict != ReviewerVerdictPass {
		t.Errorf("verdict = %q, want PASS (object part.text with nested .text)", got.Verdict)
	}
	if got.Source != VerdictSourceLogPartText {
		t.Errorf("source = %q, want %q", got.Source, VerdictSourceLogPartText)
	}
...
// TestExtractVerdictFromJSONL_StructuredResultObject — Finding 2 continued.
// `.result` can be a structured object with a `verdict` field, not just a
// string. The carrier walk now tries .text → .verdict → .raw_output inside
// object carriers so an adapter that emits a review object under `.result`
// is no longer dropped as no-verdict.
func TestExtractVerdictFromJSONL_StructuredResultObject(t *testing.T) {
	line, _ := json.Marshal(map[string]any{
		"type": "review_result",
		"result": map[string]any{
			"verdict": "FAIL",
			"summary": "structured FAIL under .result",
		},
		"part": map[string]any{
			"type": "review-result",
			"review": map[string]any{
	...
	// .result is priority 3, part.review.verdict is priority 6 — so .result's
	// FAIL wins (higher priority). The structured .result shape is honored
	// as a FAIL carrier.
	if got.Verdict != ReviewerVerdictFail {
		t.Errorf("verdict = %q, want FAIL (.result object with verdict field)", got.Verdict)
	}
	if got.Source != VerdictSourceLogResultField {
		t.Errorf("source = %q, want %q", got.Source, VerdictSourceLogResultField)
	}
}

// TestExtractVerdictFromJSONL_PartReviewVerdict_NonSynthetic — Finding 3 of
// the gastown-wisp-53s rejection. For NON-synthetic review_result entries,
// .part.review.verdict is the authoritative reviewer verdict and must be
// honored as a carrier (priority 6). The previous implementation skipped
// this path entirely, masking legitimate structured FAILs when no other
// carrier yielded a verdict.
func TestExtractVerdictFromJSONL_PartReviewVerdict_NonSynthetic(t *testing.T) {
	// Non-synthetic review_result: real verdict=FAIL at the top level, no
	// synthetic markers (no adapter_error, no canonical synthetic
	// summary/explanation). No part.text, no findings[].raw_output. The
	// ONLY parseable verdict lives at .part.review.verdict.
	line, _ := json.Marshal(map[string]any{
		"type": "review_result",
		"part": map[string]any{
			"type": "review-result",
	...
	if got.Verdict != ReviewerVerdictFail {
		t.Fatalf("verdict = %q, want FAIL (non-synthetic .part.review.verdict must be honored)", got.Verdict)
	}
	if got.Source != VerdictSourceLogPartReviewVerdict {
		t.Errorf("source = %q, want %q", got.Source, VerdictSourceLogPartReviewVerdict)
	}
}

// TestExtractVerdictFromJSONL_PartReviewVerdict_SkippedForSynthetic — Finding 3
// negative path. For synthetic wrappers, .part.review.verdict is a fixed
// FAIL wrapper signal that masks the real model verdict inside
// findings[].raw_output. The synthetic path must take findings[].raw_output
// instead, recovering the model's real PASS.
func TestExtractVerdictFromJSONL_PartReviewVerdict_SkippedForSynthetic(t *testing.T) {
	line, _ := json.Marshal(map[string]any{
		"type": "review_result",
		"part": map[string]any{
			"type": "review-result",
	...
	if got.Verdict != ReviewerVerdictPass {
		t.Fatalf("verdict = %q, want PASS (synthetic wrapper must use findings[].raw_output, not the FAIL wrapper verdict)", got.Verdict)
	}
	if got.Source != VerdictSourceLogPartReviewFindingsRawOutput {
		t.Errorf("source = %q, want %q", got.Source, VerdictSourceLogPartReviewFindingsRawOutput)
	}
...
// TestIsSyntheticReviewResult covers the three synthetic-detection signals
// individually: adapter_error field, canonical failure summary, canonical
// failure explanation in any finding. Each alone is sufficient to mark a
// record synthetic.
func TestIsSyntheticReviewResult(t *testing.T) {
	baseReview := func() map[string]any {
		return map[string]any{
			"verdict":  "FAIL",
			"summary":  "real failure summary",
			"findings": []any{},
		}
	}
	cases := []struct {
		name string
		mod  func(review map[string]any)
		want bool
	}{
		{"no marker", func(r map[string]any) {}, false},
		{"adapter_error marker", func(r map[string]any) {
			r["adapter_error"] = "no_parseable_review_json"
		}, true},
		{"canonical summary marker", func(r map[string]any) {
			r["summary"] = "OpenCode review output was normalized to FAIL because it did not contain parseable {verdict, findings, summary} JSON."
		}, true},
		{"canonical explanation marker", func(r map[string]any) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			entries := parseLogLines(t, string(line)+"\n")
			if len(entries) == 0 {
				t.Fatal("no entries parsed")
			}
			got := isSyntheticReviewResult(entries[0])
			if got != c.want {
				t.Errorf("isSyntheticReviewResult = %v, want %v", got, c.want)
			}
		})
	}
}
