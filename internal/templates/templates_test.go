...
func TestRenderRole_Refinery_DurableGateBeforeDirectPush(t *testing.T) {
	...
	required := []string{
		...
		"Start it detached with setsid nohup",
		"setsid nohup bash -c",
		"run exactly the poll/verify/push block below",
		"GATE_WRITER_FILE=\"/tmp/refinery-gate-${MERGE_TREE}.writer\"",
		"durable gate writer file missing",
		"gate still running; check again later",
		"grep -q '^GATE_EXIT=0$'",
		"grep -q '\"decision\":\"MERGE\"'",
		"hmac.compare_digest",
