...
// MergeSlotAcquire call (from any actor — a current waiter or a newcomer)
// will then take the freed slot. We deliberately do NOT auto-promote
...
// slot to it strands the slot under a holder that never called Acquire. The
// holder-vs-waiters split is enforced by Acquire itself, which removes the
// caller from Waiters when it successfully takes the slot.
