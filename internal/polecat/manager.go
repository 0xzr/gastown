...
func (m *Manager) workstateInputForPolecat(name string, state State, issue string) WorkstateInput {
	...
	// State. LivenessSignals is fail-closed: a tmux/PID error means the
	// gatherer cannot prove the session is gone, so we must NOT drop the
	// error silently. Mark LivenessCheckFailed so DecideWorkstate blocks
	// SAFE_TO_NUKE / reusable while liveness is uncertain (gastown-9rl,
	// codex blocking finding).
	if running, heartbeatFresh, heartbeatExists, processAlive, livenessErr := m.LivenessSignals(name); livenessErr != nil {
		input.LivenessCheckFailed = true
		input.LivenessCheckFailedReason = livenessErr.Error()
	} else {
...
//
// A confirmed absent session (HasSession returned false cleanly) is reported as
// a clean all-false result with err=nil. We do not probe PID/heartbeat for a
// known-absent session; otherwise a normal absent-session/no-heartbeat case
// would surface GetPanePID's "no such session" error as a liveness tool
// failure instead of a clean dead signal (gastown-9rl, codex warning).
func (m *Manager) LivenessSignals(name string) (sessionRunning, heartbeatFresh, heartbeatExists, processAlive bool, err error) {
	...
	sessionName := session.PolecatSessionName(session.PrefixFor(m.rig.Name), name)
	running, herr := m.tmux.HasSession(sessionName)
	if herr != nil {
	...
	if !running {
		// Confirmed absent session: clean dead signal, not an error.
		return false, false, false, false, nil
	}
