...
// isSessionProcessAlive returns true only when we can positively confirm the
// agent process is alive. It is the fail-closed counterpart to
// isSessionProcessDead: if liveness cannot be determined (tmux errors, missing
// PID, etc.) it returns false rather than assuming alive. This prevents dead
// polecats from being classified as live (gastown-cet.9, gastown-9rl).
func isSessionProcessAlive(t *tmux.Tmux, sessionName string, townRoot string) (bool, error) {
	...
	// Fallback: PID signal probing only when no heartbeat exists.
	if t == nil {
		return false, nil
	}
	pidStr, err := t.GetPanePID(sessionName)
...
func (m *Manager) workstateInputForPolecat(name string, state State, issue string) WorkstateInput {
	...
	// workstate classifier can distinguish a live agent from stale persisted
	// State. Errors are intentionally fail-closed: missing signals keep the
	// classifier in its conservative state-based fallback (gastown-cet.9,
	// gastown-9rl).
