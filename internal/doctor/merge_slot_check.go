...
// gastown-p61un a corrupt Description cannot be safely self-healed by
// rewriting from a known-good initial value (it's a torn-write-corruption
// state), but we CAN replace the corrupted blob with this empty shell. The
// slot then reports as available and any subsequent Acquire creates a fresh,
// consistent state. The previous (unparseable) holder/waiter information is
// lost; this is the documented behavior of "recreate the slot bead fresh"
// referenced in the bead notes.
	...
	for _, rigDir := range rigDirs {
		...
		for _, issue := range issues {
			...
			if seenSlot {
				...
				// a data hygiene problem; the safe behavior is to treat the
				// first one we encounter as canonical and flag the others.
...
// recovery is operator-visible (matches the visibility pattern in
// PrefixMismatchCheck / DatabasePrefixCheck).
