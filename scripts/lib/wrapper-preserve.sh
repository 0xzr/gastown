...
gt_install_canary() {
    ...
    # live slot. `timeout` is GNU coreutils (present on the operational
    # hosts); if it is absent, fall back to an unbounded probe rather than
    # silently skipping the gate (fail-open on the timer, fail-closed on
    # the exit code).
    if command -v timeout >/dev/null 2>&1; then
        ...
        if ! "$elf" version >/dev/null 2>&1; then
            echo "gt_install_canary: $elf version probe failed" >&2
            return 1
        fi
    fi
    return 0
...
_gt_install_preserve_wrapper_body() {
    ...
    # bad or hung build aborts BEFORE the live binary is touched. Fail-closed
    # on the exit code; on a timer failure (no `timeout` binary) we still
    # require a clean `version` exit rather than skipping the gate.
