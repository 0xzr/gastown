...
#     snapshot.
#   - Freezes the chosen snapshot to a staged copy, then runs the canary on
#     those exact staged bytes. The staged copy is what gets renamed into the
#     live slot, so a mutable explicit restore path cannot change between
#     canary and restore.
...
_gt_install_rollback_body() {
    ...
    # Freeze the candidate BEFORE touching the live slot, then canary the
    # exact bytes that will be restored. A previous version copied $restore to
    # a canary probe, then copied $restore a second time to the live staging
    # file. If $restore was mutable or symlinked, the second copy could differ
    # from the canary-approved bytes. This single staged copy closes that
    # rollback TOCTOU.
    ...
    # Atomic restore: rename the already-canaried staged copy over the live
    # slot. Rename is atomic on POSIX filesystems, and no second copy is taken
    # from the restore path after the canary verdict.
